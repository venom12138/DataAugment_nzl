#用于curriculum learning的baseline++,分三阶段训练
import argparse
import os
import shutil
import time
...
parser.add_argument('--descrip', default='', type=str,
                    help='description of experiment')

parser.add_argument('--aug_mode', default='', type=str,
                    help='mixup or prob')
...
def main():
    ...
    wandb.init(project="curriculum-project", config = args)
    time1 = time.localtime()
    wandb.run.name = 'curriculum_'+str(args.descrip) +str(time1.tm_year)+str(time1.tm_mon)+str(time1.tm_mday)+str(time1.tm_hour)+str(time1.tm_min)
    ...
    #     datasets.__dict__[args.dataset.upper()]('/home/yu-jw19/venom/ISDA-for-Deep-Networks/data', train=False, 
    ...
    for epoch in range(start_epoch, training_configurations[args.model]['epochs']):
        ...
        if epoch < args.aug_epoch:
            train_loader = torch.utils.data.DataLoader(
                                ...
                                batch_size=args.CIFAR_Batch, shuffle=True, num_workers = 1, pin_memory = True,
                                drop_last=args.drop_last)
            train(train_loader, model, fc, ce_criterion, optimizer, epoch, scheduler=scheduler)
        elif epoch < args.aug_epoch +30:
            train(mytraindata_loader(args, 0.5), model, fc, ce_criterion, optimizer, epoch, scheduler=scheduler)
        else :
            train(mytraindata_loader(args, 0.0), model, fc, ce_criterion, optimizer, epoch, scheduler=scheduler) ##isda->ce:yjw 0529
...
def mytraindata_loader(args, aug_magnitude):
    normalize = transforms.Normalize(mean=[x / 255.0 for x in [125.3, 123.0, 113.9]],
                                        std=[x / 255.0 for x in [63.0, 62.1, 66.7]])
    if args.aug_mode == 'prob':
        transform_train = transforms.Compose([
            transforms.RandomCrop(32, padding=4),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
        ...
        augmentpolicy = aug_lib.RandAugment(n = 2, m = 14, Res = 0, lambda0 = 0, aug_mode = 'prob')
        ...
        augmentpolicy = aug_lib.RandAugment(n = 2, m = 14, Res = 1, lambda0 = aug_magnitude, aug_mode = 'mixup')
