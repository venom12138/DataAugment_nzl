...
from mydataset import myCIFAR100
...
import aug_lib_1014 as aug_lib
...
file = '/home/yu-jw19/venom/ISDA-for-Deep-Networks/CIFAR/predictions'
if not os.path.exists(file):
    os.makedirs(file)
if not os.path.exists(file+'/train_raw'):
    os.makedirs(file+'/train_raw')
if not os.path.exists(file+'/validate_raw'):
    os.makedirs(file+'/validate_raw')
if not os.path.exists(file+'/validate_aug'):
    os.makedirs(file+'/validate_aug')
# if not os.path.exists('/home/yu-jw19/venom/ISDA-for-Deep-Networks/CIFAR/predictions/train_raw.csv'):
#    os.make
def main():
    ...
    wandb.init(project="test-project", group = 'aug_raw_kl_div', config = args)
    time1 = time.localtime()
    wandb.run.name = 'aug_raw_kl_div'+ str(time1.tm_year)+str(time1.tm_mon)+str(time1.tm_mday)+str(time1.tm_hour)+str(time1.tm_min)
    ...
    #     drop_last=False)
    train_loader = torch.utils.data.DataLoader(
        myCIFAR100('/home/yu-jw19/venom/ISDA-for-Deep-Networks/data', train=True, download=True,
        transform=transform_train),
    ...
    val_loader = torch.utils.data.DataLoader(
        ...
        batch_size=args.CIFAR_Batch, shuffle=False, num_workers = 1, pin_memory = True,
        drop_last=False)
    ...
    augmentpolicy = aug_lib.RandAugment(n = 2, m = 14, Res= 0, lambda0 = 0)
    aug_transform_test.transforms.insert(0, augmentpolicy)
    aug_transform_test.transforms.append(aug_lib.cutoutdefault(16))  
    aug_val_loader = torch.utils.data.DataLoader(
        datasets.__dict__[args.dataset.upper()]('/home/yu-jw19/venom/ISDA-for-Deep-Networks/data', train=False, transform=aug_transform_test),
        batch_size=args.CIFAR_Batch, shuffle=False, num_workers = 1, pin_memory = True,
        drop_last=False)
    ...
    for epoch in range(start_epoch, training_configurations[args.model]['epochs']):
        ...
        prec1 = validate(val_loader, aug_val_loader, model, fc, ce_criterion, epoch)
...
def train(train_loader, model, fc, criterion, optimizer, epoch, scheduler=None):
    ...
    train_raw = torch.tensor([]).cuda()
    train_index = torch.tensor([]).cuda()
    ...
    for i, (x, target, index) in enumerate(train_loader):
        ...
        index = index.cuda()
        train_raw = torch.cat((train_raw, output),dim = 0)
        # print(train_raw.shape)
        train_index = torch.cat((train_index, index),dim = 0)
        ...
        if (i+1) % args.print_freq == 0:
            ...
            fd.write(string + '\n')
            fd.close()
    sorted_logits, indices = train_index.sort(dim=0)
    sorted_output = train_raw.index_select(dim = 0, index=indices)
    with open('/home/yu-jw19/venom/ISDA-for-Deep-Networks/CIFAR/predictions/train_raw/'+str(epoch)+'.npy', 'w') as f1: 
        np.save('/home/yu-jw19/venom/ISDA-for-Deep-Networks/CIFAR/predictions/train_raw/'+str(epoch)+'.npy', sorted_output.detach().cpu().numpy().tolist())

def validate(val_loader, aug_val_loader, model, fc, criterion, epoch):
    """Perform validation on the validation set"""
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    # aug_losses = AverageMeter()
    aug_p = torch.tensor([]).cuda()
    raw_p = torch.tensor([]).cuda()
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        with torch.no_grad():
            features = model(input_var)
            output = fc(features)
            raw_p = torch.cat((raw_p, output),dim = 0)
    ...
    # aug_val_loader
    for i, (input, target) in enumerate(aug_val_loader):
        target = target.cuda()
        input = input.cuda()
        input_var = torch.autograd.Variable(input)
        target_var = torch.autograd.Variable(target)
        ...
        with torch.no_grad():
            features = model(input_var)
            output = fc(features)
            aug_p = torch.cat((aug_p, output),dim = 0)
    ...
    logp_aug = F.log_softmax(aug_p, dim=-1)
    p_raw = F.softmax(raw_p, dim=-1)
    kl_mean = F.kl_div(logp_aug, p_raw, reduction='sum')/10000
    wandb.log({'klmean':kl_mean})

    with open('/home/yu-jw19/venom/ISDA-for-Deep-Networks/CIFAR/predictions/validate_raw/'+str(epoch)+'.npy', 'w') as f1:  
        np.save('/home/yu-jw19/venom/ISDA-for-Deep-Networks/CIFAR/predictions/validate_raw/'+str(epoch)+'.npy', raw_p.detach().cpu().numpy().tolist())
    with open('/home/yu-jw19/venom/ISDA-for-Deep-Networks/CIFAR/predictions/validate_aug/'+str(epoch)+'.npy', 'w') as f1:  
        np.save('/home/yu-jw19/venom/ISDA-for-Deep-Networks/CIFAR/predictions/validate_aug/'+str(epoch)+'.npy', aug_p.detach().cpu().numpy().tolist())
    f2 = open('/home/yu-jw19/venom/ISDA-for-Deep-Networks/CIFAR/predictions/kl_div.txt', 'a+')
    f2.write(str(float(kl_mean.detach().cpu()))+'\n')
    f2.close()
    return top1.ave
