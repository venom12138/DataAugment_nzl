#用于curriculum learning的baseline
import argparse
import os
import shutil
import time
...
def main():
    ...
    wandb.run.name = 'curriculum'+str(args.descrip) +str(time1.tm_year)+str(time1.tm_mon)+str(time1.tm_mday)+str(time1.tm_hour)+str(time1.tm_min)
    ...
    for epoch in range(start_epoch, training_configurations[args.model]['epochs']):
        ...
        # train for one epoch
            ...
            train(aug_train_loader, model, fc, ce_criterion, optimizer, epoch, scheduler=scheduler) ##isda->ce:yjw 0529
