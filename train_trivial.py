...
parser.add_argument('--numstrengths', default='31', type = int,
                    help='hyper-parameter for set augmentation space')
# drop_last
parser.add_argument('--drop_last', default= False, type = int,
...
# Trivialaugment
parser.add_argument('--Trivialaugment', dest='Trivialaugment', action='store_true',
                    help='whether to use Trivialaugment')
parser.set_defaults(Trivialaugment=False)
# Trivialaugmentbase
parser.add_argument('--Trivialaugmentbase', dest='Trivialaugmentbase', action='store_true',
                    help='whether to use Trivialaugmentbase')
parser.set_defaults(Trivialaugmentbase=False)
...
record_path = './ISDA test/' + str(args.dataset) \
              ...
              + ('_trivialaugment_' if args.Trivialaugment else '')\
              + ('Trivialaugmentbase' if args.Trivialaugment else '')\
              + ('_cos-lr_' if args.cos_lr else '')\
              + ('Mean' if args.Mean else '')\
              + ('STD' if args.STD else '')\
              + 'augmentationspace_'+str(args.augmentationspace)\
              + 'numstrengths'+str(args.numstrengths)\
...
def main():
    ...
    if args.augment:
        if args.autoaugment:
            ...
            print("Trivialaugment")
            augmentpolicy = aug_lib.TrivialAugment()
            pre_augmentpolicy = aug_lib.UniAugmentWeighted(0, [0, 1.0])
            #cutoutpolicy = aug_lib.CutoutTrivial(16)
            transform_train = transforms.Compose([
                pre_augmentpolicy,
        ...
        elif args.Trivialaugmentbase:
            print("Trivialaugmentbase")
            augmentpolicy = aug_lib.TrivialAugment()
            pre_augmentpolicy = aug_lib.UniAugmentWeighted(0, [0, 1.0])
            #cutoutpolicy = aug_lib.CutoutTrivial(16)
            transform_train = transforms.Compose([
    ...
    for epoch in range(start_epoch, training_configurations[args.model]['epochs']):
        ...
        # train for one epoch
        train(train_loader, model, fc, ce_criterion, optimizer, epoch) ##isda->ce:yjw 0529
