...
class RandAugment:
    def __init__(self, n = 2, m = 10, Res = False, lambda0 = 0.0, aug_mode = ''):
        self.n = n
        self.m = m  # [0, 30]
        self.Res = Res # True or False
        assert (lambda0 <= 1 and lambda0 >= 0)
        ...
        if self.Res:
            ...
            for op in ops:
                ...
                if random.random() < prob:
                    ...
                    img2 = t(img_after)
                    # prob2 = random.random()
                    img3 = (self.lambda0*img1 + (1-self.lambda0)*img2)
                    img = t2(img3)
                # file = '/home/yu-jw19/venom/ISDA-for-Deep-Networks/CIFAR/savedimgs'
                # if not os.path.exists(file):
                #     os.makedirs(file)
                # self.i += 1
                # img.save(os.path.join(file,str(self.i) + ".jpg"))
        else:
            if self.aug_mode == 'prob':
                p = random.random()
                if p < 1-self.lambda0:
                    for op in ops:
                        prob = np.random.uniform(low=0.2, high=0.8) 
...
class GaussAugment:   
    def __call__(self, img):
        # prob = np.random.uniform(low=0.2, high=0.8) 
        # if random.random() < prob:
        img2 = transform.ToTensor(img)
        print(img2) 
            # img = torch.min(img + torch.normal(0, 3*torch.ones(img.size()))
        return img